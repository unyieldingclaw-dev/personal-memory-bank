...
- Non-blocking follow-ups tracked, not yet done: `CHANGELOG.md` entry for the Codex gate add/remove; a migration-section caveat in `CURSOR-VS-CLAUDE.md` + `docs/SETUP-GUIDE.md:294`'s stale link text; test coverage for the adapters' `pre`/no-argument fail-open path; `AGENTS.md`'s remaining Claude-Code-only claims at :82/:154 (Codex-facing vs tool-general is unresolved); the pre-compact-check gate-message wording now contradicts `AGENTS.md:39`'s manual-diagnostic instruction in the Codex context (no gate exists there to pause).
