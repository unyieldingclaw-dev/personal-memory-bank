...
**Still pending:** the marker-destruction-on-denial fix itself (now with a concrete live repro to work from); atomic version-cache write + `diff_hash()` trap-scoping fix; final full test-suite run + re-run `/change-review` + push/merge PR #8.
