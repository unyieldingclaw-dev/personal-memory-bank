...
- New packages or imports that may not exist — verify they are real (hallucinated dependencies / slopsquatting)
- Prompt injection vulnerabilities in code that constructs LLM prompts or processes LLM output
- Insecure deserialization or blind trust of AI/external service responses
...
- Hallucinated imports or references to functions/libraries that do not exist
- Unnecessary abstraction layers or wrapper classes that add no value (common AI over-engineering pattern)
- Redundant boilerplate that AI introduced which duplicates existing utilities
