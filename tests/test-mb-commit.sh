...
if command -v pwsh > /dev/null 2>&1; then
    # `cygpath -w` is REQUIRED, not cosmetic. $REPO_ROOT comes from Git Bash's `pwd`, so it is a
    # POSIX path (/c/Users/...) that PowerShell cannot resolve. An unresolvable script makes pwsh
    # print "not recognized as a name of a cmdlet" and STILL EXIT 0 -- so the happy-path
    # assertions below (which want rc=0 and no "subworktree" in the output) both pass while
    # executing nothing. A draft of this block did exactly that. Hence the hard gate below.
    ...
    # HARD GATE, not a warning. An earlier form incremented FAIL and then fell through into the
    # assertions anyway -- so a run could print "parity assertions would pass vacuously" and, one
    # line below, two PASSes for the very assertions it had just declared meaningless. `exit
    # $LASTEXITCODE` yields 0 when no native process ever ran ($LASTEXITCODE is $null), which is
    # what made those passes possible. Verified: `pwsh -Command 'exit $null'` exits 0.
    probe=$(MB_SCRIPT="$MBPS1" pwsh -NoProfile -Command '& $env:MB_SCRIPT help' 2>&1)
    probe_rc=$?
    if [ "$probe_rc" -ne 0 ] || printf '%s' "$probe" | grep -qiE 'not recognized as (a|the) name'; then
        # Both conditions, because neither alone is sufficient: the text is English- and
        # version-specific (Windows PowerShell 5.1 says "as THE name ... operable program"), and
        # the exit status alone would not catch a pwsh that loads the script but fails inside it.
        echo "  FAIL: pwsh cannot run $MBPS1 — parity assertions SKIPPED rather than passed vacuously"
        FAIL=$((FAIL + 1))
    else
...
if (cd "$TMPDIR_BADIDX" && git status --porcelain memory-bank > /dev/null 2>&1); then
    ...
    if command -v pwsh > /dev/null 2>&1 && [ -n "${MBPS1:-}" ]; then
