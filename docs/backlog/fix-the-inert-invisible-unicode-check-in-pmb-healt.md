...
FIXED 2026-09-09 by `1f7a7e7`, with detection-efficacy coverage added in `tests/test-baseline-health.sh` blocks 8 and 9. `status:` stayed `open` from 2026-09-09 to 2026-09-21 because `mb`'s backlog schema then had only `open`/`promoted`/`dismissed`, and none of those meant "fixed directly": `dismissed` would have asserted this was dropped rather than done, and an invented value would have failed `promote`'s validation. That was the real gap, not this fix — closed 2026-09-21 by adding a `resolved` status and `mb backlog resolve` command (`docs/superpowers/specs/2026-07-14-backlog-design.md`), applied here via `mb backlog resolve fix-the-inert-invisible-unicode-check-in-pmb-healt`.
