...
A parser cannot resolve shell variables or read scripts, so the command-text layer cannot close this on its own. This is the gap [NS-26] names ("nothing currently enforces the review gate for a commit/push typed directly in a human terminal"), and the same text-matching class as [NS-25]/[NS-43]. The git-level layer that NS-26 Bundle A plans (a marker check in the git pre-commit/pre-push hooks) sees the real commit however it was spelled. Until then: tell dispatched agents that a hook denial means stop and report, and check `git rev-parse HEAD` / `git status` after every agent returns.
