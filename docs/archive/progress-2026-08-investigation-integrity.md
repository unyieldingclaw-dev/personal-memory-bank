# Archived Progress — Investigation-Integrity Mechanism (2026-08-12 through 2026-08-14)
...
open was dropped — see `activeContext.md`'s `[NS-16]`/`[NS-17]`/`[NS-21]` for current state.
...
- 📌 Disclosed, not fixed (non-blocking, deferred given time/token constraints on this session): no automated regression test exists for `scripts/warn-stale-review-marker.sh`/`.ps1` — this matches, rather than introduces, an existing gap (`check-contract.sh`/`.ps1`, the hook this one is explicitly modeled on, is also untested; this repo's test suite covers only hooks that can `deny`/block, not warn-only ones). The new hook is also not yet documented in `docs/HOOKS-GUIDE.md` (every other registered hook has a numbered section there) — a real discoverability gap, somewhat ironic given `[NS-21]`'s own subject matter, deferred for the same reason. **Superseded 2026-08-17/18:** this hook is now documented in `HOOKS-GUIDE.md` — Task #31, committed `7d6d2b9`; see the live `progress.md`'s condensed 2026-08-12-through-14 pointer entry for the current-state summary (this archive predates that commit).
- 📌 Not yet committed — this diff now spans the hook, its template distribution, two NS-21 doc fixes plus template mirror, and this memory-bank update, reviewed via one lighter single-agent verification pass rather than the full 5-domain process, given explicit token constraints for this session — a deliberate, disclosed scope reduction from the standing "no lite path" rule, not a silent one. **Superseded:** committed since — see live `progress.md`.
...
- 📌 The plan itself is fixed and self-consistent (verified: sequential step numbering per task, balanced code fences) but **not yet executed** — no code changes made, plan file still untracked. Still needs a Subagent-Driven vs. Inline execution decision. See `[NS-15]`. **Superseded 2026-08-17:** all 14 tasks implemented and committed — see live `progress.md`.
...
- 📌 **2026-08-13 update:** implementation plan written and self-reviewed —
  ...
  **Superseded 2026-08-17:** all 14 tasks implemented and committed.
