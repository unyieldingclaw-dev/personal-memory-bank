# CHANGELOG archive — 0.1.0 (2026-04-29) through 1.2.1 (2026-08-03)

Released version history, moved out of `CHANGELOG.md` on 2026-09-07.

**Why it moved:** `CHANGELOG.md` had reached 901 lines against the 800-line hard cap enforced by
`.github/workflows/pmb-health.yml`'s File Size job, which fails the build on `pull_request` to
`main`. It had been sitting at 792 lines — eight lines of headroom — so any entry of real length
would have broken CI. Trimming a single entry would have re-armed that for the next author, so the
released history moved instead.

**Verified safe before moving:** no script, workflow or hook reads `CHANGELOG.md`. Checked by grep
across `*.sh`, `*.ps1`, `*.yml` and `*.md`; the only references are historical plan documents.

`CHANGELOG.md` keeps `[Unreleased]` and the pending section. Anything already released lives here.
Keep it that way rather than re-inlining released sections.
