...
Early drafts of this design conflated "add a checklist" with "fix the confidence-calibration problem." They are not the same fix, and treating them as one is exactly the ambiguity the user caught. Three separate mechanisms are needed, because they catch three different failure classes:
...
**3. Independent review discipline (a separate agent, not a self-administered pass, before executing a plan with real consequence).** Grounding and coverage discipline both happen inside the same reasoning process that produced the work being checked — even a genuinely adversarial self-review shares the blind spots of the agent performing it. This is not hypothetical: on 2026-08-12/13, a 14-task implementation plan for this repo's own review-gate mechanism passed its author's self-review (which itself found 3 real gaps), then a separately-dispatched agent with no context from writing the plan found 8 more real, file:line-verified defects in the same document, including a Blocking-severity bug (a `set -e` crash on a path the plan itself was supposed to fix). See `memory-bank/progress.md`'s 2026-08-14 entry for the full account.

This mechanism is **advisory, like the other two — it does not gate phase progression.** `memory-bank/projectbrief.md` fixes this repo's workflow at 7 phases as a non-negotiable requirement; this mechanism is deliberately scoped as a recommended practice inserted between existing phases, not an 8th phase, matching the precedent `.claude/commands/change-review.md`'s own "Step 3.5: Baseline Repo Health" already sets for a non-counted, informational step. Findings are surfaced and strongly recommended for action, not enforced as a hard block — the same trust model this whole skill already operates under (see "Known Limitations" below).

**Trigger:** any implementation plan (regardless of which skill produced it) about to be handed to execution (`subagent-driven-development`, `executing-plans`, or any other multi-task execution handoff) — including a plan being *resumed* for execution after having sat untouched, since staleness against the current repo state is itself a real risk this mechanism catches.

**Mechanism:** dispatch a fresh `Agent` instance with **no conversation context** from writing the plan, using a capable model — e.g. Sonnet or higher, **never a cost-optimized/cheap model** (this project's `.claude/settings.json` defaults subagents to a cost-optimized model; both `/code-review` and `/change-review` already override this default for their own review subagents for the same reason — see `code-review.md`'s Step 5 and `change-review.md`'s Job 9). Give it a self-contained prompt with the spec, the plan, and instructions to independently verify — not just re-read — every claim against the actual current repo state: cross-reference the plan against its spec file by file; verify dual-shell (bash/PowerShell) parity wherever the plan touches paired implementations; **trace error-handling/control-flow behavior** (`set -e`/`set -u`/pipefail semantics, what happens when a command mid-script fails) for any new or modified shell script — presence of a matching implementation is not the same as correctness under failure; check task ordering and inter-task dependencies; verify any `templates/` mirror the plan touches is actually kept in sync; verify the plan's claimed CI/test integration points against the actual current CI/test files, not assumed; and directly verify every `file:line` claim the plan makes against the real file. Report findings using this repo's existing `standards/CODE-REVIEW.md` vocabulary (`VERIFIED`/`INFERRED`/`SPECULATIVE` basis, `Severity` + `Blocking` fields) rather than inventing a new schema.

**Recommended resolution:** any finding with `Blocking: true` should be fixed in the plan before execution begins. Non-blocking findings should be disclosed transparently in the plan's own Design Note (matching the existing convention already used for self-review gaps), not silently dropped — but per the advisory framing above, proceeding without full resolution is a judgment call the user can make, not a hard stop.

**If the review agent itself fails to complete** (spend limit, crash, timeout — this happened for real on the case motivating this design): retry once. If it still cannot complete, disclose the blocker plainly to the user and get an explicit decision before proceeding without either a completed review or an informed choice to skip it.

All three are necessary. Grounding without coverage still misses whole categories. Coverage without grounding can still assert unverified conclusions confidently within the categories it does check. Neither grounding nor coverage, however adversarially self-administered, escapes the blind spots of the agent that produced the work being checked — independent review is the only one of the three performed by a different reasoning process entirely.

### Anti-theater safeguard (named explicitly, using this conversation as the worked example)

A checklist — even the three-mechanism version above — can be satisfied by going through the motions: write four section headers, assert each was "considered," move on. This is not hypothetical; it is what happened with the word "checklist" itself earlier in this exact design process. The skill's own instructions must name this directly, the same way `docs/HOOKS-GUIDE.md`'s worked examples name real incidents rather than describing failure modes abstractly. Structural defense against it: coverage-discipline output isn't a checkbox list, it's the same `VERIFIED`/`INFERRED`/`SPECULATIVE` grounding applied to *each category checked* — "I considered X" is not itself a claim with a basis; "I checked X by reading file Y, here's what I found" is. This doesn't make theater impossible (a sufficiently unwilling instance of me could still fabricate a plausible-looking citation) — it makes theater *visible and spot-checkable* rather than indistinguishable from genuine work, the same residual-risk shape this repo already accepted for the review-gate's self-attestation limits.
...
2. **The three-mechanism structure** (grounding discipline, coverage discipline, independent review discipline) as portable *principles*, with the `VERIFIED`/`INFERRED`/`SPECULATIVE` vocabulary offered as one working example, not a mandate — the receiving system may already have its own equivalent classification worth reusing instead.
...
4. **Regression test for independent review discipline itself**: a planted-gap plan fixture (mirroring item 2's approach) where the plan's own self-review section claims a gap was fixed but the actual task content still contains it — confirm a dispatched independent review catches the discrepancy.
...
5. **Independent review has its own cost and failure modes.** Dispatching a fresh agent costs real time and tokens, and can itself fail to complete (see the explicit fallback rule under mechanism 3) — it is not a free or infallible check, just a structurally different one from self-review. And because it is advisory, not gating (see mechanism 3's projectbrief.md note), nothing stops execution from proceeding without it if the user chooses to.
...
- Modifying `/code-review` or `/change-review` themselves — independent review discipline is a new, complementary mechanism for plans, not a change to how diffs are reviewed.
- Making independent review discipline a gating phase — deliberately kept advisory to avoid conflicting with `memory-bank/projectbrief.md`'s non-negotiable 7-phase workflow requirement.
...
| `docs/WORK-MB-INVESTIGATION-BRIEF.md` | New, then later Modified to add mechanism 3 — the portable, PMB-agnostic investigation brief for the user's separate work Memory Bank, placed at the docs/ root (not `docs/superpowers/`, which is reserved for PMB's own design artifacts) since this file is meant to be extracted and carried elsewhere |
...
| `standards/WORKFLOW.md` | New "Phase 3.5 — Independent Plan Review" section, advisory/non-gating (does not change the 7-phase count) — non-renumbering insert, precedented by `change-review.md`'s "Step 3.5"; Quick Reference and Claude Code Integration tables updated |
| `templates/standards/WORKFLOW.md` | Phase 3.5 + the same two table updates, manually applied at the equivalent position — NOT a blind copy, since this file has pre-existing, unrelated drift from the live file in its Phase 3 content |
| `.claude/commands/feature-dev.md` | Short advisory note added between the Phase 3 and Phase 4 paragraphs, matching the file's terse per-phase style; explicitly notes it is not one of the 7 counted phases and is not subject to the file's "do not skip phases" instruction |
| `templates/claude-commands/feature-dev.md` | Mirror (confirmed byte-identical to the live file before this change) |
