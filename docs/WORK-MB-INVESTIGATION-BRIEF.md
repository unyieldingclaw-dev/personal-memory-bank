...
## Part 2: The two-mechanism fix

Two distinct mechanisms are required, because they catch two different failure classes. A single "be more thorough" instruction, or a single checklist, catches neither reliably — this was tested directly in the same session (see Part 4, worked example 3).
...
Both are necessary. Grounding without coverage still misses whole categories that were never mentioned. Coverage without grounding can still assert unverified conclusions confidently within the categories it does check.
