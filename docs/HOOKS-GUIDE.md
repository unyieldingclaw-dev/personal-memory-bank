...
- Git commits without running `pre-commit` at all in `git rebase --continue` and `git cherry-pick --continue` (both measured). `git revert --continue` and `git am` are expected to behave the same way but were not measured.
