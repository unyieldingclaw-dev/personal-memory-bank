...
**Tag pushes are exempt from the push marker (added 2026-08-03):** reported live by the `ai-code-review-agent` session cutting release `v1.8.0` — `git tag v1.8.0 origin/main && git push origin v1.8.0` was denied outright, including the harmless `git tag` half, because `needs_push` detection only checked the git subcommand (`push`), never what ref was being pushed. The gate's actual check for a push — a diff-hash between `origin/main` and `HEAD` — is a meaningful "was this code reviewed" proxy for a branch push; a tag carries no diff at all, so satisfying the gate meant hashing whatever unrelated branch happened to be locally checked out. `tag_only_push()` (`.sh`, python3) / `Test-TagOnlyPush` (`.ps1`, native) close this by resolving the pushed ref via git ground truth — **never a naming heuristic**, since `git push origin v1.8.0` is syntactically identical whether `v1.8.0` resolves to a tag or a branch — clearing `needs_push` only when every `git push` invocation in the (possibly compound) command targets a ref that's either an already-existing tag (`git show-ref`, requiring the tag exist WITHOUT a same-named branch also existing) or one this same compound command is itself creating via a `git tag <name> ...` half (the tag doesn't need to already exist locally, since creating one is itself just labeling an existing commit, never introducing new code — this is what makes the exact reported scenario exemptable even though the tag genuinely doesn't exist yet when the hook fires, before either half of the compound command has run). Every ambiguous or unrecognized shape — 0/1/3+ positional args after `push`, `--delete`, `--tags` mixed with an explicit refspec, a ref that's neither existing nor about-to-be-created — falls through to "not exempt," the same "over-trigger, never under-gate" direction this file uses everywhere; an unrecognized shape only ever costs an unnecessary re-review, never grants an unearned exemption. The exemption only runs on the high-confidence, JSON-parsed command-text path (never the raw-stdin fallback), since granting it from untrustworthy text would be the one place in this file where a false read makes the gate *less* safe rather than just triggering an occasional unnecessary re-review. See `docs/superpowers/specs/2026-08-03-review-gate-tag-push-exemption-design.md` for the full design writeup and rejected alternatives (a release-aware ancestor check was considered and rejected — it can't cover the tag-doesn't-exist-yet case the report actually hit). Only the `PreToolUse` hook needed this change; the `PostToolUse` companion's existing "reissue only if a pending-push-presha file exists" check already no-ops correctly for an exempted push, since the `PreToolUse` hook never wrote one.

**CRITICAL bypass found and fixed during opposition review, before this feature was ever pushed:** the first version of this exemption validated a pushed refspec by splitting on `:` and checking only the SOURCE side (`git push <remote> <src>:<dst>` pushes the LOCAL ref `src` to the REMOTE ref `dst` — the ref that actually changes on `origin` is `dst`, not `src`). Since any existing tag name satisfied the check regardless of which side it appeared on, `git push origin <any-existing-tag>:main` was wrongly exempted — the tag name checked out as safe while the actual effect was overwriting `main` with unreviewed content, with no marker required and no deny at all. Reproduced empirically end-to-end, including with `--force`, landing an unreviewed commit directly on a scratch repo's `main`. This required no adversarial crafting: it works with any tag a repository already has, which is nearly all of them. Correctly resolving `dst` would require querying the REMOTE's own ref-name disambiguation (an unqualified `dst` can resolve differently depending on what already exists there) from inside a synchronous `PreToolUse` hook with no network round-trip available — rather than approximate that with a local heuristic that's still fundamentally guessing at remote state, the fix is simpler and safer: **any refspec containing a colon is never exempt, full stop**, regardless of what the source side resolves to. An agent that needs to push an explicit `src:dst` tag refspec (an unusual shape) just goes through the normal `/change-review` flow — the same "unrecognized shape costs an unnecessary re-review, never grants an unearned exemption" direction this whole exemption already commits to elsewhere.

**MEDIUM-HIGH bypass found by a SECOND opposition-review pass, also fixed before this feature was ever pushed:** even after the colon-refspec fix above, `created_tag_names`/`is_existing_tag` were trusted purely on the tag NAME resolving safely — with zero check on WHAT COMMIT the tag actually pointed at. `git tag foo <any-unreviewed-local-commit> && git push origin foo` was exempted regardless of whether that commit had ever been reviewed. This doesn't overwrite any branch (confirmed empirically — a tag push alone never moves a branch ref), but DOES unconditionally upload that commit's objects to the remote's object store with zero review — a real, distinct bypass of this review gate's core purpose, just not the branch-overwrite flavor the first pass found. Fixed by requiring the tagged commit to already be an ancestor of `origin/main` (i.e., already reviewed and published via the ordinary commit+push gate) before granting the exemption, via `is_ancestor_of_main()` (`.sh`) / `Test-AncestorOfMain` (`.ps1`) — applied to BOTH the same-command tag-creation case (checking the ref being tagged, e.g. `positional[1]` of `git tag <name> <ref>`, or `HEAD` if omitted) and the already-existing-tag case (checking what the existing tag resolves to via `git rev-parse ...^{commit}`, peeling annotated tags). This still correctly exempts the originally reported bug's exact scenario — tagging `origin/main` itself is trivially its own ancestor — without reopening the "tag doesn't exist yet when this hook fires" problem, since the check resolves the REF BEING TAGGED, not the tag itself. Fails closed (not exempt) if `origin/main` doesn't resolve at all, rather than falling back to `HEAD` or any other ref — a fallback would weaken the exact property this check exists to enforce, since `HEAD` itself may contain unreviewed local commits.

**HIGH bypass found by a THIRD opposition-review pass, also fixed before this feature was ever pushed:** even after the ancestor-check fix above, `is_ancestor_of_main()`/`Test-AncestorOfMain` trusted the LOCAL `refs/remotes/origin/main` tracking ref as ground truth for "already reviewed and on the remote" — but that ref is an ordinary local file, freely rewritable via `git update-ref refs/remotes/origin/main <any-sha>` (or a direct edit to `.git/refs/remotes/origin/main` / `.git/packed-refs`), none of which this hook gates at all (it only intercepts `git commit`/`push`/`gh pr merge`). Reproduced directly: rewriting the local tracking ref to point at an unreviewed commit made the ancestor check wrongly report that commit as already-reviewed, letting a tag pointing at it through — reopening the exact gap the ancestor-check fix was meant to close, just one step of indirection removed, and requiring no more than one ordinary, easily-overlooked git command first. Fixed by running a fresh, explicit `git fetch origin '+refs/heads/main:refs/remotes/origin/main' --quiet` immediately before the ancestor check in both `is_ancestor_of_main()` and `Test-AncestorOfMain`, overwriting any locally-tampered or simply-stale value with the actual remote's current state before trusting it. If the fetch itself fails (offline, auth failure, no route to `origin`), this fails closed — no exemption — rather than falling back to the now-known-untrustworthy pre-fetch value: the `git push` this hook is pre-authorizing needs that exact same network access to `origin` the moment it actually runs, so requiring it here first adds no new failure mode beyond what the underlying command already needs. This does mean every tag-push-exemption check now costs one network round-trip to `origin`, accepted as the cost of the check actually meaning what it claims.
