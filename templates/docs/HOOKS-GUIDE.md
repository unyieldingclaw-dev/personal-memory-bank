...
The Stop hook is excluded from `templates/.claude/settings.json` because it causes indefinite hangs in headless/remote sessions (no user present to dismiss the notification, stalling the session permanently). If you need a Stop notification in an interactive-only project, add it to that project's local `.claude/settings.json` manually.

### 3. Contract Scope Check (`PreToolUse` — Write + Edit tools)

Checks whether a file being written is within the scope declared in the active task contract (`.claude/contracts/active-task.json`). Implemented in `scripts/check-contract.ps1` and `scripts/check-contract.sh`. See `docs/CONTRACTS-GUIDE.md` for the full contract schema.
...
**Tag pushes are exempt from the push marker.** A pushed ref is resolved via git ground truth (`git show-ref`), never a naming heuristic — `git push origin v1.8.0` is syntactically identical whether `v1.8.0` is a tag or a branch. `needs_push` is cleared only when every `push` invocation in the command targets a ref that's either an existing tag (with no same-named branch also existing) or one the same compound command is itself creating via its own `git tag <name> ...` half — and in both cases, only when the tagged commit is already an ancestor of `origin/main` (already reviewed and published), checked via a fresh `git fetch origin main` followed by `git merge-base --is-ancestor`. Any `src:dst` refspec (with a colon) is never exempt regardless of what `src` resolves to — `git push <remote> <src>:<dst>` actually updates the remote ref `dst`, not `src`. Three real bypasses were caught by three rounds of opposition review before this feature was ever pushed: branch overwrite via `src:dst`, unreviewed-content publication via an unverified tag target, and trusting a locally-tamperable `origin/main` tracking ref instead of fetching fresh — each closed before shipping. Any other ambiguous or unrecognized shape also falls through to "not exempt."
