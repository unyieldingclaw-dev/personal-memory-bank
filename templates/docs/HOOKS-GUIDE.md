...
- **`pre-commit` runs only for `git commit` itself** (githooks(5)), including a `git commit` or `git merge --continue` that concludes a conflicted merge (measured). Every other command that creates commits skips it: a clean `git merge` or `git pull`, `git cherry-pick`, `git revert`, `git rebase` and their `--continue` forms, `git am` (which runs `pre-applypatch` instead), and plumbing such as `git commit-tree` with `git update-ref`. Measured: the clean merge and both `--continue` forms; the rest are from git's documentation.
- A clean merge can only bring in the merged branch's content, since `git merge` refuses to start with changes staged (git-merge(1), which allows narrow exceptions for some merge strategies), so it is not a way to author `memory-bank/` changes. The one difference from the conflicted path: files git combines from both sides are refused there but committed by a clean merge, which can only happen when this branch already carries committed `memory-bank/` changes. `git revert` is not like this: reverting a main commit that touched `memory-bank/` writes new `memory-bank/` content in the worktree, unchecked, and rolls main back when the branch merges.
